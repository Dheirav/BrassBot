...
def game():
    ...
    serve.GAMES.clear()
    g = {"lock": __import__("threading").Lock(), "room": "main", "name": "Tester"}
    serve.GAMES["main"] = g
    serve.start(g, seed=21, players=4, opponent="heuristic")
    return g
...
def test_a_stale_move_index_is_refused(game):
    ...
    stale = game["version"]
    serve.record(game, legal_actions(game["state"])[0])
    serve.advance(game)
    assert game["version"] != stale
...
def test_a_whole_game_plays_and_every_snapshot_is_serialisable(game):
    ...
    while not game["state"].finished and steps < 400:
        json.dumps(serve.snapshot(game))          # would raise on a bad payload
        actions = legal_actions(game["state"])
        if not actions:
            break
        serve.record(game, actions[0])
...
def test_export_writes_a_log_the_parsers_can_read(game, tmp_path, monkeypatch):
    ...
    play_out(game)
    path = Path(serve.export_log(game))
