...
render() {
  ...
  local line done total t rate left eta pct bar filled unit
  ...
  unit=$(sed -n 's/.*unit=\([a-z]*\).*/\1/p' <<<"$line"); unit=${unit:-steps}
  ...
  printf "  [%-30s] %s%%  %s/%s %s   %.0fm elapsed   %s/min   ETA %s\n" \
         "$bar" "$pct" "$done" "$total" "$unit" \
         "$(awk -v t="$t" 'BEGIN{printf "%.0f", t/60}')" "$rate" "$eta"
  grep '^RESULT' "$log" 2>/dev/null | tail -3 | sed 's/^RESULT /  done: /'
  grep -E '^  [a-z_]+ +[-0-9.]' "$log" 2>/dev/null | tail -4 | sed 's/^/  /'
}
